...
def calculate_peaks(src):
    fft_size = 1024

    processor = AudioProcessor(src, fft_size, numpy.hanning)
    samples_per_loop = processor.audio_file.nframes / float(fft_size)

    peak_data = []

    for x in range(fft_size):
        seek_point = int(x * samples_per_loop)
        next_seek_point = int((x + 1) * samples_per_loop)
        peaks = processor.peaks(seek_point, next_seek_point)
